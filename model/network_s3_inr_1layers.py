...
class double_u_net_skip(nn.Module):
    def __init__(self, Out_fhsi, Out_fmsi, args):
        ...
        self.INR2D_1 = INR2D(dim=240, out_dim=240, hidden_dim=256, hidden_layers=args.inr2d_mlp_layers, L=args.inr2d_pe_len, sigma=args.inr2d_gauss)
        self.INR1D_1 = INR1D(self.band, self.scale1[0], 256, args.inr1d_mlp_layers, args.inr1d_pe_len, sigma=args.inr1d_gauss)
    ...
    def forward(self, Out_fhsi, Out_fmsi):
        ...
        x4 = up(x3)
        x5 = x4 + hsi_inr_1
        out_fhsi = self.ex3(torch.cat([s1, x5], dim=1))
        ...
        # nn.Upsample(scale_factor=2, mode=upsample_mode[i])
        # down
        x6 = self.ex4(Out_fmsi)

        # x2=fun.interpolate(x1,self.scale[1],mode='bilinear')
        x7 = nn.AdaptiveAvgPool2d(self.scale[1])(x6)
        x7_high = nn.AdaptiveMaxPool2d(self.scale[1])(x6)
        msi_inr_1 = self.INR2D_1(x7_high)
        ...
        out_fmsi = self.ex6(torch.cat([s2, x9], dim=1))
