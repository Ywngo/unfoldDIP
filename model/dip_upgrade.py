...
class LinearUnmixingProjection(nn.Module):
    ...
    def __init__(self, args, in_channels_unet):
        ...
        self.spatial_net = Spatial_Abundance_Net(args, in_channels_unet, feat_dim=32)
        self.spectral_net = Spectral_Endmember_Net(args, in_channels_unet, feat_dim=32)
        # �������� CoreMatrixGenerator ������������
    ...
    def forward(self, S_k, coords_2d, coords_1d, target_shape):
        ...
        # V ����: (B, L, R)
        V = self.spectral_net(coords_1d, S_k)
...
# =========================================================================
class UnrolledUnmixingFusion(nn.Module):
    def __init__(self, args, L_bands):
        ...
        self.projectors = nn.ModuleList([
            LinearUnmixingProjection(args, L_bands) for _ in range(self.K)
        ])
...
class dip():
    def __init__(self, args, psf, srf, blind):
        ...
        self.net = UnrolledUnmixingFusion(args=self.args, L_bands=L_bands).to(args.device)
    ...
    def train(self):
        ...
        for epoch in range(1, self.args.niter3_dip + self.args.niter_decay3_dip + 1):
            ...
            for i, X_pred in enumerate(out_list):
                ...
                loss_step += getattr(self.args, 'sam_weight', 0.01) * self.angle_similarity_loss(lr_hsi_pred,
                                                                                                 self.lr_hsi)
            ...
            if epoch % 50 == 0:
                with torch.no_grad():
                    ...
                    print(
                        f'?? [Probe-Gradient] Data Grad Norm: {last_grad_norm:.4f} | Prior Residual Norm: {last_res_norm:.4f}')
